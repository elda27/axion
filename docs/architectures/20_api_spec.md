...
| GET    | `/v1/aggregations/{aggregationId}/run-metrics?key=`           | 集計内 RM 一覧      |
...
## Run Metric（RM）/ DerivedMetric（RMの一種）

> **重要**: RMとCIは絶対に混ぜない（APIも分ける）

| Method | Endpoint                                                | 説明              |
| ------ | ------------------------------------------------------- | ----------------- |
| GET    | `/v1/runs/{runId}/run-metrics`                          | Run単位           |
| GET    | `/v1/batches/{batchId}/run-metrics?key=&limit=&cursor=` | Batch単位（任意） |

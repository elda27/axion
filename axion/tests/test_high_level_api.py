...
from axion.high_level import create_artifact, create_run, set_active_run
...
def _env_context(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AXION_ORG", "Org-A")
    monkeypatch.setenv("AXION_PROJECT", "Project-A")
    monkeypatch.setenv("AXION_BATCH", "Batch-A")
...
def test_create_run_requires_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("AXION_ORG", raising=False)
    monkeypatch.delenv("AXION_PROJECT", raising=False)
    monkeypatch.delenv("AXION_BATCH", raising=False)
    client = _FakeClient(org_items=[], project_items=[], batch_items=[])

...
Axion SDK (`axion.AxionClient`) の使い方を示すサンプルスクリプト集です。

## 前提条件

- Axion サーバーが起動していること（デフォルト: `http://localhost:8000`）
- Python 3.12+
...
| 変数名           | デフォルト値            | 説明                   |
| ---------------- | ----------------------- | ---------------------- |
| `AXION_BASE_URL` | `http://localhost:8000` | Axion API のベース URL |

```bash
AXION_BASE_URL=http://localhost:8000 uv run python examples/quickstart.py
```

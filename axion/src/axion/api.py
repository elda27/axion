...
from axion.schemas import ArtifactResponse, RunResponse

AXION_ORG_ENV = "AXION_ORG"
AXION_PROJECT_ENV = "AXION_PROJECT"
AXION_BATCH_ENV = "AXION_BATCH"
...
@contextmanager
def _borrow_client(client: AxionClient | None):
...
def _load_context_names() -> _ContextNames:
    org = os.environ.get(AXION_ORG_ENV)
    project = os.environ.get(AXION_PROJECT_ENV)
    batch = os.environ.get(AXION_BATCH_ENV)
    missing = [
        env_name
        for env_name, value in (
            (AXION_ORG_ENV, org),
            (AXION_PROJECT_ENV, project),
            (AXION_BATCH_ENV, batch),
        )
        if not value
    ]
    if missing:
...
def _get_or_create_org_id(client: AxionClient, org_name: str) -> str:
...
def _get_or_create_project_id(
    client: AxionClient, org_id: str, project_name: str
) -> str:
    cursor: str | None = None
    while True:
...
def _get_or_create_batch_id(
    client: AxionClient, project_id: str, batch_name: str
) -> str:
    cursor: str | None = None
    while True:

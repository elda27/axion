# Axion Lab GCP Deployment

This directory contains all configuration and scripts needed to deploy Axion Lab on Google Cloud Platform.
...
│  │                    Compute Engine VM                            │  │
│  │  ┌──────────┐  ┌────────────────┐  ┌─────────────────────────┐ │  │
│  │  │  nginx   │──│ axion-lab-backend  │──│  Cloud SQL Auth Proxy   │ │  │
│  │  │ (proxy)  │  │   (FastAPI)    │  │                         │ │  │
│  │  └────┬─────┘  └────────────────┘  └───────────┬─────────────┘ │  │
│  │       │                                         │               │  │
│  │  ┌────┴─────┐  ┌────────────┐                   │               │  │
...
| `DB_USER`                        | Database username          | `axion_lab`                                   |
| `DB_PASSWORD`                    | Database password          | Your secure password                          |
| `DB_NAME`                        | Database name              | `axion_lab`                                   |
...
| `GCE_INSTANCE` | VM instance name      | `axion-lab-vm`                     |
| `GCE_ZONE`     | GCE zone              | `asia-northeast1-a`                |
| `GCS_BUCKET`   | Artifacts bucket name | `your-project-axion-lab-artifacts` |
| `APP_DOMAIN`   | Application domain    | `axion-lab.example.com`            |
...
| axion-lab-backend | 8000 | FastAPI backend (internal only)          |
...
For staging VMs, ensure the `axion-lab-web-dev` tag is applied to enable port 6006 access:

```bash
gcloud compute instances add-tags axion-lab-vm \
  --zone=asia-northeast1-a \
  --tags=axion-lab-web-dev
```
...
```bash
gcloud compute ssh axion-lab-vm --zone=asia-northeast1-a
```
...
```bash
# Backend logs
sudo journalctl -u axion-lab-backend -f
...
```bash
gcloud compute ssh axion-lab-vm --zone=asia-northeast1-a --command="
  sudo rm -rf /opt/axion_lab
  sudo mv /opt/axion-lab-backup /opt/axion_lab
  sudo systemctl restart axion-lab-backend
...
sudo systemctl status axion-lab-backend
...
psql -h localhost -U axion_lab -d axion_lab
```
...
sudo journalctl -u axion-lab-backend --since "10 minutes ago"
```
